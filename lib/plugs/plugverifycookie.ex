defmodule Jwt.Plugs.VerifyCookie do
    ...
    def init(opts) do
        case Enum.count(opts) do
          3 -> opts
          _ -> [Verification.default_options().ignore_token_expiration, Verification.default_options().time_window, default_verification_failure_response()]
        end
    end
    ...
    defp continue_after_verification({:error, _}, conn, %{"location" => location, "response" => :redirect}) do
        conn
         |> resp(:found, "")
